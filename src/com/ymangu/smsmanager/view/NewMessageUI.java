...
public class NewMessageUI extends Activity implements OnClickListener {
	...
	private void initView() {
		...
		actvNumber.setAdapter(mAdapter);   //为自动提示框设置适配器
		/**
		 * 功能： 过滤联系人，将结果提供给自动提示框
		 **/
	...
	public void onClick(View v) {
		...
		case R.id.ib_new_message_select_contact:		// 选择联系人
			Intent intent = new Intent(Intent.ACTION_PICK);  //"android.intent.action.PICK"
			intent.setData(Contacts.CONTENT_URI);    //"content://com.android.contacts/contacts"
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
		if(requestCode == 100 && resultCode == Activity.RESULT_OK) {
			...
			if (contactID != -1) {
				String address = Utils.getContactAddress(getContentResolver(),
						contactID);
				if (!TextUtils.isEmpty(address)) {
					actvNumber.setText(address);
					etContent.requestFocus();   //选择联系人之后 ，将光标移动到输入信息框
				}
			}
