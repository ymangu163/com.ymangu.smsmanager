...
/**
 * 功能：实现输入一些字符后 出现自动提示,查询我们这人Provider
 **/

public class SearchableProvider extends SearchRecentSuggestionsProvider {
	private final String AUTHORITY = "com.ymangu.smsmanager.provider.SearchableProvider";
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		Cursor cursor = getContext().getContentResolver().query(Sms.SMS_URI,
				sms_projection, where, null, null);

		if (cursor != null && cursor.getCount() > 0) {

			/*
			 * 定义出matrix游标结果集, 并指定其列 _ID, SUGGEST_COLUMN_TEXT_1(显示在上面: 号码),
